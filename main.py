...
while run:
    if phase == 'prepare':
        ...
        mode, active, phase, e_str = 1, None, 'main', [str(i) for i in var]
    time.tick(30)
    ...
    for event in pygame.event.get():
        ...
        if phase == 'main':
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                gx, gy = x // 20, y  // 20 - 2
                if mode > 1 or not (0 <= gx < col and 0 <= gy < row): continue
            if click(1):
                if mode:
                    safe = [[x, y] for x in range(col) for y in range(row) if [x, y] not in gcn(gx, gy)]
                    mine, mode = random.sample(safe, minenum), 0
                ...
                if not mode and minenum == sum(c in ' #?' for r in show for c in r): mode, timer = 3, now
            if click(3): show[gx][gy] = {'#': '?', '?': ' ', ' ': '#'}.get(show[gx][gy], show[gx][gy])
    ...
    if phase == 'main':
        ...
        screen.blit(title[mode - 1 if mode else 0], (10, 10))
        ...
        if mode > 1 and now - timer > 3000: phase = 'ask'
